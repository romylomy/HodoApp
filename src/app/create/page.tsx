...
export default function CreatePage() {
  ...
  const {data: products, refetch} = useQuery({
    ...
    queryFn: async () => {
      const {data} = await axios.post<QueryResult<bikeRoute>[]>(
        'https://hodo-app.vercel.app/api/products', 
        {
          filter:{
            sort: filter.sort,
 ...
 const _debouncedSubmit = useCallback(debouncedSubmit, [])
  ...
  const TRAILTYPE_FILTERS = {
    id: 'trailtype',
    name: 'trailtype',
    options: [
      { value: 'single', label: 'Single' },
      { value: 'double', label: 'Double' },
      { value: 'downhill', label: 'Down Hill' },
    ] as const,
  }
        ...
        <div className="flex items-center">
          ...
          <DropdownMenuContent align='end'>
            ...
            {SORT_OPTIONS.map((option)=>(
              ...
              className={cn('text-left bg-slate-50 w-full block px-4 py-2 text-sm', {
    ...
    <div className='grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4'>
            ...
            <Accordion type='multiple' className='animate-none'>
              ...
              <Accordion type='multiple' className='animate-none'>
                ...
                <AccordionItem value='difficulty'>
                  <AccordionTrigger className='py-3 text-sm text-gray-400 hover:text-gray-500'>
                    <span className='font-medium text-gray-900'>Trail Type</span>
                  </AccordionTrigger>

                  <AccordionContent className='pt-6 animate-none'>
                    <ul className='space-y-4'>
                      {TRAILTYPE_FILTERS.options.map((option, optionIdx) => (
                        <li key={option.value} className='flex items-center'>
                          <input
                            type='checkbox'
                            id={`trailtype-${optionIdx}`}
                            onChange={() => {
                              applyArrayFilter({
                                category: 'trailtype',
                                value: option.value,
                            ...
                            checked={filter.trailtype.includes(option.value)}
                            className='h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500'
                          />
                          <label
                            htmlFor={`color-${optionIdx}`}
            ...
            <ul className='z-[-10] lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8'>
